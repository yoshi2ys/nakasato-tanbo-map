...
- **地図**: MapLibre GL JS + 国土地理院「電子国土基本図（オルソ画像）」タイル
  `https://cyberjapandata.gsi.go.jp/xyz/ort/{z}/{x}/{y}.jpg`（API キー不要、出典表記必須、ズーム 18 まで）
  - 「全国最新写真（シームレス）」より鮮明。同一タイル（z18）で 15.7KB に対して 59.5KB あり、畦道と圃場の境が読み取りやすい。
  - z19 以上は国土地理院に存在しない（404）。Esri World Imagery はこの地域の z19 以上が「データなし」。より高解像度が要るなら Mapbox 衛星か市町村の公開画像。
...
- 自動検出精度は畦道のコントラスト依存。検出結果は常に「編集可能な下書き」として Step 3 に合流させ、手動調整で回収する設計。
  - 実測: 大区画の整理田（新潟市南区）では 4/5 が妥当な面積で検出できるが、中山間地の小区画・不整形圃場（十日町）では 0/6。収穫後の裸地は道路や畦と色が近く、フラッドフィルが止まる境界がない。
  - 許容差と平滑化を上げれば十日町でも当たるようになるが、整理田では 1 枚あたり 4〜6ha に膨れて漏れる（許容差 24 + ブラー 5 で実測）。地点差をパラメータ 1 組で吸収するのは無理なので、現行値（許容差 18・ブラー無し）のまま据え置き、十日町では手動描画で対応する。
