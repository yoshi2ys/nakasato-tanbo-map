...
- モデル: `items.ts` — 田んぼ・計測・ピンを `Item` 1 つの型でまとめる。保存も書き出しも GeoJSON（`localStorage` の `tanbo-map.paddies`、書き込みは 400ms のデバウンス）。設定は別キー `tanbo-map.settings`（`settings.ts`）。フォルダは `Item.folder`（1 段だけ、未設定は「未分類」）で、一覧の並びは登録順ではなく名前順（`Intl.Collator('ja', { numeric: true })`）。
